...
MR<-"/Volumes/X10Pro/data/synergistic/mr_res"; strip<-function(x)sub("[.].*","",x); nk<-function(x)gsub("[^a-z0-9]","",tolower(x))
...
comp<-fread("/Users/cjh/Documents/GitHub/HumanDisease-FertilityPleiotropy/data/rhoge_auth_compare.csv")
compS<-rbind(comp[,.(dk,mr=M_MR,fs=M_FS,sex="Male")],comp[,.(dk,mr=F_MR,fs=F_FS,sex="Female")])
...
rA<-Aant[,{rw<-cov.wt(cbind(ldsc,pct),wt=n,cor=T)$cor[1,2];pw<-tryCatch(coef(summary(lm(pct~ldsc,weights=n)))[2,4],error=function(e)NA);.(lab=sprintf("%s: r=%.2f, P=%.1e",sex,rw,pw))},by=sex]
