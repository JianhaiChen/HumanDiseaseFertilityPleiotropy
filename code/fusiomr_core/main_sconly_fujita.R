...
# clump_param = list(clump_kb = c(100,10,100), clump_r2 = c(0.1,0.01,0.01))
# ctypes_short = c("end","ast","exc","inh","mic","opc","oli")
ctypes_short = c("ast","exc","oli")
diseases_short = "ad"
# for (k in 1:3) {
...
dat1 = fread(paste0('harmo_fujita/eqtl_', ctypes_short[i], '_', chr, '.txt.gz')) %>% dplyr::filter(se != Inf, se_out != Inf, se > 0, se_out > 0) %>% drop_na() 
sumtbl1 = NULL
# aa = dat1 %>% group_by(gene_name) %>% summarise(n = n()); summary(aa$n)
gene_list1 = unique(dat1$gene_id)
...
sumtbl1 = do.call(rbind, pblapply(seq_along(gene_list1), function(t) process_gene_sc_only_fujita(dat1, gene_list1, t, pcut, clump_kb, clump_r2, b_gamma_prior_mean, b_theta_prior_mean)))
fwrite(sumtbl1, paste0("sumtbl_fujita/sconly_", diseases_short, "_", pcut, "_", clump_kb, "_", clump_r2, "_", ctypes_short[i], "_", chr, '.txt'))
} 
