...
SC<-"/Volumes/X10Pro/data/synergistic/mr_res"                                      # recomb maps: SC/recomb/
CFPATH<-"/Volumes/X10Pro/data/synergistic/mr_res/session_2026-07-09_iv_filter/colleague_shared_iv_filter.csv"
MY<-"/Volumes/S840/04mypaper/lin/conflictresolution/fusio2trait"
...
rhop<-dcast(rge,disease~sex,value.var="p_ge");setnames(rhop,c("M","F"),c("pgeM","pgeF"))  # rho-GE OWN significance (cor.test over ng genes)
...
nk<-function(x)gsub("[^a-z0-9]","",tolower(x))   # unify disease-name matching (nk) across all merges
rate<-CF[,.(n=.N,pct=100*mean(combo%in%c("++","--"))),by=.(dk=nk(disease),sex=fifelse(fitness=="male","M","F"))][n>=1]  # all 97; low-n de-emphasized by size & n-weighted regression
## ---- category map ----
...
pA1<-ggplot(dbL,aes(corr,pct,color=sex,fill=sex))+geom_point(aes(size=n),alpha=.5)+geom_smooth(aes(weight=n,linetype=sex),method="lm",formula=y~x,se=T,linewidth=.8,alpha=.12)+
  ...
  labs(x=expression("disease-fertility "*rho[GE]*" (expression effect)"),y="% antagonistic (indep. loci)")+th+theme(legend.position=c(.995,.02),legend.justification=c(1,0),legend.background=element_rect(fill=alpha("white",.7),color=NA),legend.box="horizontal")
...
pA2<-ggplot(convL,aes(rg,ge,color=sex))+geom_hline(yintercept=0,linetype=2,color="grey85")+geom_vline(xintercept=0,linetype=2,color="grey85")+
  ...
  labs(x=expression(italic(r)[g]*" (LDSC, genome-wide SNPs)"),y=expression(rho[GE]*" (MR, expression effect)"))+th+theme(legend.position="none")
pB<-pA2|pA1
...
dC<-rbindlist(list(
  ...
  aC[,.(Suppl_Trait,cat6,xpos,sex="Male",  measure="rho_GE",corr=ge_M,sig=pgeM<0.05)],
  aC[,.(Suppl_Trait,cat6,xpos,sex="Female",measure="r_g",   corr=rg_F,sig=abs(zF)>1.96)],
  aC[,.(Suppl_Trait,cat6,xpos,sex="Female",measure="rho_GE",corr=ge_F,sig=pgeF<0.05)]))
...
dC[,grp:=fifelse(!sig|is.na(sig),"n.s.",fifelse(measure=="r_g","genome-wide SNPs (r_g, LDSC)","expression effect (rho-GE)"))]
dC[,grp:=factor(grp,levels=c("genome-wide SNPs (r_g, LDSC)","expression effect (rho-GE)","n.s."))]
...
ggsave("/Volumes/X10Pro/data/synergistic/mr_res/Fig_BC_final.pdf",fig,width=182,height=185,units="mm",device=cairo_pdf)
...
cat("Panel A diseases:",uniqueN(rate$dk)," | r_g diseases:",nrow(rgt)," | rho-GE diseases:",nrow(rho),"\n")
