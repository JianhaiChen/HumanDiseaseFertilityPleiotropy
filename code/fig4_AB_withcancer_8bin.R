...
bin6<-function(P,rmap,lab){P[,gg:=rmap[a]*rmap[b]];P<-P[is.finite(gg)&gg!=0];oi<-which(P$gg<0);si<-which(P$gg>0);P[,grp:=NA_integer_]
 P$grp[oi]<-as.integer(cut(P$gg[oi],quantile(P$gg[oi],seq(0,1,.25)),include.lowest=T));P$grp[si]<-4L+as.integer(cut(P$gg[si],quantile(P$gg[si],seq(0,1,.25)),include.lowest=T))
 P[,grpf:=factor(grp,levels=1:8)];H<-P[,.(med=median(revrate),n=.N),by=grp][order(grp)];lb<-c("opposite\n(strong)","","","opposite\n(weak)","same\n(weak)","","","same\n(strong)")
 ggplot(P,aes(grpf,revrate))+geom_hline(yintercept=.5,linetype="dotted",color="grey65")+geom_violin(aes(fill=grp<=4),color=NA,alpha=.26,scale="width",width=.85)+
  geom_boxplot(width=.17,outlier.size=.4,outlier.alpha=.25,linewidth=.35,fill="white")+geom_point(data=H,aes(grp,med),color="#C0392B",size=1.9)+
  ...
  labs(x=expression("fertility coupling of disease pairs   (opposite  "*rho[i]%.%rho[j]*" < 0  "%->%"  same  "*rho[i]%.%rho[j]*" > 0)"),y="opposite-effect proportion (% shared genes)")+
  theme_classic(base_size=7)+theme(axis.text.x=element_text(size=5.5),axis.text.y=element_text(size=6),axis.title.y=element_text(size=6.5),axis.title.x=element_text(size=6,margin=margin(t=3)),plot.margin=margin(18,16,10,0))}
build<-function(sex,out){R<-pipe(sprintf("ACAT_%s_gene_disease_all.csv",sex),sex);gA<-saddle_tri(R$P,R$rmap);gB<-bin6(copy(R$P),R$rmap,ifelse(sex=="female","Female (maternal)","Male (paternal)"))
 m<-plot_grid(gA,gB,ncol=2,rel_widths=c(1.06,0.92),labels=c("A","B"),label_x=c(0.02,0.005),label_size=11,align="h",axis="t")   # smaller B frame; A keeps height via coord_fixed
 ggsave(paste0(BD,out,".png"),m,width=182,height=88,units="mm",dpi=300,bg="white");ggsave(paste0(BD,out,".pdf"),m,width=182,height=88,units="mm",device=cairo_pdf,bg="white");cat(sex,"->",out,"\n")
...
build("female","Fig_intralocus_MAIN_AB_female_withcancer_8bin")
build("male","Fig_intralocus_SUPP_AB_male_withcancer_8bin")
